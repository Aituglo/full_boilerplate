...
export function* loadLoginUser() {
  ...
  try {
    ...
    if (result && result.status === 'success') {
      ...
      yield put(loginUserError('app.global.error'));
    }
  } catch (error) {
    yield put(loginUserError('app.global.error'));
  }
  ...
  try {
    if (register.password === register.verifPassword) {
      ...
      if (result && result.status === 'success') {
        ...
        yield put(registerUserError('app.global.error'));
      }
    } else {
      yield put(registerUserError('app.auth.password_mismatch'));
    }
  } catch (error) {
    yield put(registerUserError('app.global.error'));
  }
...
export function* loadManageUser() {
  ...
  try {
    ...
    if (result && result.status === 'success') {
      ...
      yield put(manageUserError('app.global.error'));
    }
  } catch (error) {
